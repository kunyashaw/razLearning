<template>
  <div class="space-y-6">
    ...
    <!-- 牛津树级别介绍弹窗 -->
    <div v-if="showOxfordInfo" class="fixed inset-0 bg-black/50 flex items-center justify-center z-50 p-4" @click.self="showOxfordInfo = false">
      <div class="bg-white rounded-2xl shadow-xl max-w-lg w-full max-h-[80vh] overflow-y-auto">
        <div class="p-6">
          <div class="flex justify-between items-center mb-4">
            ...
            <button @click="showOxfordInfo = false" class="text-gray-400 hover:text-gray-600 text-2xl">×</button>
          </div>
          <div class="space-y-3 text-sm">
            <div class="p-3 rounded-xl bg-green-50">
              <strong class="text-green-600">L1-L2级：</strong>
              <span class="text-gray-600">入门阶段，每本16页，每页1-3个单词，英语启蒙</span>
            </div>
            <div class="p-3 rounded-xl bg-green-50">
              <strong class="text-green-600">L3-L4级：</strong>
              <span class="text-gray-600">基础阶段，每本16-24页，简单句式入门</span>
            </div>
            <div class="p-3 rounded-xl bg-green-50">
              <strong class="text-green-600">L5-L6级：</strong>
              <span class="text-gray-600">提升阶段，每本24页，日常对话和简单故事</span>
            </div>
            <div class="p-3 rounded-xl bg-green-50">
              <strong class="text-green-600">L7-L9级：</strong>
              <span class="text-gray-600">进阶阶段，每本24页，词汇量1000+，较长故事</span>
            </div>
            <div class="p-3 rounded-xl bg-green-50">
              <strong class="text-green-600">L10-L12级：</strong>
              <span class="text-gray-600">高级阶段，每本约28页，初章书水平</span>
            </div>
          </div>
          <div class="mt-4 pt-4 border-t">
            <p class="text-xs text-gray-500 text-center">牛津树以Kipper一家人的生活故事为主线，趣味性强</p>
          </div>
        </div>
      </div>
    </div>
          ...
          <div class="p-3">
            ...
            <div class="flex justify-between items-center">
              ...
              <div v-if="isDevMode" class="flex gap-1">
                ...
                <a 
                  v-if="book.pdf && book.pdf.relativePath" 
                  :href="'/' + book.pdf.relativePath" 
                  target="_blank"
...
const showOxfordInfo = ref(false)
const jumpPage = ref('')
...
const lastReadBook = computed(() => {
  return bookStore.getLastReadBook('oxford')
})
...
const continueReading = () => {
  if (lastReadBook.value) {
    const books = oxfordService.getAllBooks('oxford')
...
const sortedLevels = computed(() => {
  return [...levels.value].sort((a, b) => {
    // L1 始终排在最前面
    if (a.name === 'L1') return -1
    if (b.name === 'L1') return 1
    // 其他按字母顺序
...
onMounted(() => {
  const oxfordBooks = oxfordService.getAllBooks('oxford')
  const levelStats = {}
  
  oxfordBooks.forEach(book => {
  ...
  // 恢复保存的级别，如果没有则默认选择L1
  const savedLevel = localStorage.getItem('oxfordSelectedLevel')
  if (savedLevel && levelStats[savedLevel]) {
    selectedLevel.value = savedLevel
  } else if (levelStats['L1']) {
    selectedLevel.value = 'L1'
  } else {
...
const allBooks = computed(() => {
  return oxfordService.getAllBooks('oxford')
})

const allTags = computed(() => {
  let allTagsList
  if (selectedLevel.value === 'all') {
    allTagsList = oxfordService.getAllTags('oxford')
  } else {
    allTagsList = oxfordService.getFilteredBooks('oxford', selectedLevel.value).flatMap(book => book.tags || [])
  }
...
const filteredBooks = computed(() => {
  ...
  // 如果不是选择"所有"，则按级别筛选
  if (selectedLevel.value !== 'all') {
    result = oxfordService.getFilteredBooks('oxford', selectedLevel.value)
  }
...
const getTagCount = (tag) => {
  ...
  if (selectedLevel.value === 'all') {
    ...
    books = oxfordService.getFilteredBooks('oxford', selectedLevel.value)
  }
...
const selectLevel = (level) => {
  ...
  localStorage.setItem('oxfordSelectedLevel', level)
  activeTags.value = []
  searchQuery.value = ''
}
